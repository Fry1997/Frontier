...
import { PALETTES, HAIRS, buildSprites, makePortrait, makeThumb } from './sprites.js';
...
const RECIPE_DEFS = [
  ...
  { id: 'shelter', name: 'SHELTER', icon: 'home', costs: [['wood', 6], ['stick', 4]], place: true, desc: 'A ROOM OF YOUR OWN' },
];
...
const QUESTS = [
  ...
  { text: 'BUILD A SHELTER', done: g => g.flags.shelterBuilt },
  { text: 'STEP INSIDE. MAKE IT HOME', done: g => g.flags.home },
...
export function createGame(cfg) {
  ...
  let hairKey = store.hair || cfg.props.hair || 'scruff';
  if (!HAIRS.some(h => h.key === hairKey)) hairKey = 'scruff';
  const sfx = new SFX();
  ...
  let pal = PALETTES[palKey], sp = getSprites(palKey, hairKey);
  const portCache = {};
  const portrait = (pk, hk) => (portCache[pk + ':' + hk] ||= makePortrait(PALETTES[pk], hk));

  // ---- world ----
  const ground = [], objects = new Map(), drops = [], rabbits = [], parts = [], floats = [], shelters = [];
  const okey = (x, y) => x + ',' + y;
  for (let y = 0; y < MH; y++) {
    ground.push([]);
    for (let x = 0; x < MW; x++) {
  ...
  const player = { x: 12.5 * T, y: 9.6 * T, dir: 'down', moving: false, animT: 0, actT: 0, cookT: 0 };
  const inv = { stick: 0, stone: 0, wood: 0, meatRaw: 0, meatCk: 0, axe: false };
  const tot = { stick: 0, stone: 0, wood: 0, meatRaw: 0, meatCk: 0 };
  const flags = { fire: false, ate: false, drank: false, shelterBuilt: false, home: false };
  const G = { inv, tot, flags };
  let stage = 0, stageFlash = 0, metersOn = false, hunger = 1, thirst = 1;
  let placing = null, fireflies = [], finaleT = 0;
  let worldMin = DAY_START;
  ...
  function blockedTile(tx, ty) {
    ...
    if (o && o.type !== 'stump' && o.type !== 'door') return true;
    return false;
  }
  ...
  // ---- time ----
  const dayNum = () => 1 + Math.floor(worldMin / 1440);
  const tod = () => worldMin % 1440; // minute of day
  function smoo(a, b, x) { x = Math.max(0, Math.min(1, (x - a) / (b - a))); return x * x * (3 - 2 * x); }
  function nightAmt() {
    const t = tod();
    if (t < 300) return 1;
  ...
  const clockLabel = () => {
    const t = tod(), hh = String(Math.floor(t / 60)).padStart(2, '0'), mm = String(Math.floor(t % 60)).padStart(2, '0');
    return 'D' + dayNum() + ' ' + hh + ':' + mm;
  };
  ...
  function startFinale() {
    ...
    setTimeout(() => { if (!dead) onUi({ homeShown: true }); }, 2400);
  }
  ...
  function recipeState() {
    return RECIPE_DEFS.map(r => ({
      ...
      costs: r.costs.map(([k, n]) => ({ icon: sp.icons[k].toDataURL(), n, have: inv[k], ok: inv[k] >= n })),
  ...
  function craftItem(id) {
    ...
    if (r.place) {
      placing = r.id === 'shelter' ? { id: r.id, w: 5, h: 4 } : { id: r.id, w: 1, h: 1 };
  ...
  function placeAnchor() {
    ...
    if (player.dir === 'down') return [fx - 2, fy];
    if (player.dir === 'up') return [fx - 2, fy - 3];
    if (player.dir === 'left') return [fx - 4, fy - 1];
    return [fx, fy - 1];
  }
  function placeValid() {
    if (!placing) return false;
    const [ax, ay] = placeAnchor();
    if (ax < 1 || ay < 1 || ax + placing.w > MW - 1 || ay + placing.h > MH - 1) return false;
    const ptx = Math.floor(player.x / T), pty = Math.floor(player.y / T);
    for (let dy = 0; dy < placing.h; dy++) for (let dx = 0; dx < placing.w; dx++) {
      const tx = ax + dx, ty = ay + dy;
      const g = tileAt(tx, ty);
      if (!(g === 'd' || g[0] === 'g' || (placing.id === 'fire' && g === 'r'))) return false;
      if (objects.get(okey(tx, ty))) return false;
      if (tx === ptx && ty === pty) return false;
      for (const d of drops) if (Math.floor(d.x / T) === tx && Math.floor(d.y / T) === ty) return false;
    }
    return true;
  }
  function confirmPlace() {
    ...
    const [ax, ay] = placeAnchor();
    if (placing.id === 'fire') {
      objects.set(okey(ax, ay), { type: 'fire', lit: true });
      flags.fire = true;
      burst(ax * T + 16, ay * T + 16, [pal.smoke, pal.dirtLo], 8, { up: 30 });
    } else {
      objects.set(okey(ax + 2, ay + 1), { type: 'site', t: 0, ax, ay });
      burst(ax * T + 80, ay * T + 40, [pal.dirtLo, pal.trunkLo], 10, { up: 30 });
    }
    sfx.place(); placing = null;
    checkQuest(); pushUi();
  }
  function cancelPlace() { placing = null; sfx.ui(); pushUi(); }
  function buildShelter(ax, ay) {
    objects.delete(okey(ax + 2, ay + 1));
    for (let dy = 0; dy < 4; dy++) for (let dx = 0; dx < 5; dx++) ground[ay + dy][ax + dx] = 'f';
    for (let dx = 0; dx < 5; dx++) {
      objects.set(okey(ax + dx, ay), { type: 'wall', kind: 'n', face: true, w: dx === 0, e: dx === 4 });
      if (dx === 2) objects.set(okey(ax + dx, ay + 3), { type: 'door' });
      else objects.set(okey(ax + dx, ay + 3), { type: 'wall', kind: 's', face: true, w: dx === 0, e: dx === 4 });
    }
    for (let dy = 1; dy < 3; dy++) {
      objects.set(okey(ax, ay + dy), { type: 'wall', kind: 'w', w: true });
      objects.set(okey(ax + 4, ay + dy), { type: 'wall', kind: 'e', e: true });
    }
    shelters.push({ ax, ay, roofA: 1, smk: 2 });
    flags.shelterBuilt = true;
    sfx.place(); sfx.craft();
    burst(ax * T + 80, ay * T + 30, [pal.thatch, pal.thatchLo, pal.woodRing], 18, { up: 45, life: 0.9 });
    // eject player if standing where a wall appeared
    const ptx = Math.floor(player.x / T), pty = Math.floor(player.y / T);
    ...
    if (inFoot && !inRoom && !inDoor) { player.x = (ax + 2) * T + 16; player.y = (ay + 3) * T + 20; }
    checkQuest(); pushUi();
  }
  ...
  function nearRoof(sh) {
    const ptx = Math.floor(player.x / T), pty = Math.floor(player.y / T);
    return (ptx >= sh.ax + 1 && ptx <= sh.ax + 3 && pty >= sh.ay + 1 && pty <= sh.ay + 2) || (ptx === sh.ax + 2 && pty === sh.ay + 3);
  }
  ...
  function action() {
    ...
    if (c.k === 'cook') { player.cookT = 1.3; sfx.cook(); return; }
    if (c.k === 'eat') {
  ...
  function pushUi() {
    onUi({
      ...
      hairKey,
      hairs: HAIRS.map(h => ({ key: h.key, name: h.name, url: portrait(palKey, h.key) })),
      craftOpen, placing: placing ? placing.id : null,
      recipes: recipeState(),
      craftPulse: stage === 1 && !inv.axe,
    });
  }
  function saveSettings() { try { localStorage.setItem('frontier.settings', JSON.stringify({ palette: palKey, muted, hair: hairKey })); } catch (e) {} }

  // ---- update ----
  function update(dt) {
    gt += dt;
    if (phase !== 'game') return;
    worldMin += dt; // 1 real second = 1 game minute
    const nightA = nightAmt();
    // movement
    ...
    for (const [k, o] of objects) {
      ...
      if (o.type === 'site') {
        const was = o.t;
        o.t += dt;
        if (Math.floor(was / 0.55) !== Math.floor(o.t / 0.55)) sfx.chop();
        if (Math.random() < dt * 6) burst(o.ax * T + 40 + Math.random() * 80, o.ay * T + 20 + Math.random() * 60, [pal.dirtLo, pal.woodRing], 1, { up: 20, sz: 1, life: 0.4 });
        if (o.t >= 1.8) buildShelter(o.ax, o.ay);
      }
    }
    // shelters: roof fade, chimney smoke, home flag
    for (const sh of shelters) {
      const target = nearRoof(sh) ? 0.06 : 1;
      sh.roofA += (target - sh.roofA) * Math.min(1, dt * 7);
      sh.smk -= dt;
      if (sh.smk <= 0) {
        sh.smk = 1.4 + Math.random() * 1.2;
        parts.push({ x: (sh.ax + 4) * T + 13 + Math.random() * 6, y: sh.ay * T - 22, vx: (Math.random() - 0.5) * 5, vy: -11, t: 0, life: 2.2, col: pal.smoke, sz: 2, grav: -3, flut: 1 });
      }
    }
    if (!flags.home && flags.shelterBuilt && playerInside()) { flags.home = true; checkQuest(); }
    // fireflies
    if (nightA > 0.5 && fireflies.length < 16 && Math.random() < dt * 1.5) {
      const a = Math.random() * Math.PI * 2, d2 = 60 + Math.random() * 160;
      fireflies.push({ x: player.x + Math.cos(a) * d2, y: player.y + Math.sin(a) * d2, a: Math.random() * 6.3, s: 8 + Math.random() * 12 });
    }
  ...
  function respawnRabbit(r) {
    for (let tries = 0; tries < 10; tries++) {
      ...
      if (Math.hypot(x - player.x, y - player.y) > 160 && !blockedPx(x, y)) {
  ...
  function render() {
    ...
    const nightA = phase === 'game' ? nightAmt() : 0;
    const warmA = phase === 'game' ? warmAmt() : 0;
    ...
    if (placing) {
      ...
      ctx.fillRect(ax * T, ay * T, T * placing.w, T * placing.h);
      ctx.globalAlpha = pulse;
      ctx.strokeStyle = ok ? p.ui.good : p.ui.bad;
      ctx.lineWidth = 2;
      ctx.strokeRect(ax * T + 1, ay * T + 1, T * placing.w - 2, T * placing.h - 2);
      if (placing.id === 'shelter') {
        ctx.globalAlpha = 0.6;
    ...
    if (placing) {
      const [ax, ay] = placeAnchor();
      if (placing.id === 'fire') ents.push({ y: ay * T + T - 1, draw: () => { ctx.globalAlpha = 0.5; ctx.drawImage(sp.props.fireUnlit, ax * T - 2, ay * T + 2); ctx.globalAlpha = 1; } });
      else ents.push({ y: (ay + 3) * T + T - 1, draw: () => { ctx.globalAlpha = 0.45; ctx.drawImage(sp.props.shelterFrame, ax * T + 22, ay * T + 14, 116, 100); ctx.globalAlpha = 1; } });
    }
    ents.sort((a, b) => a.y - b.y);
    for (const e of ents) e.draw();
    // roofs (fade when inside)
    for (const sh of shelters) {
      if (sh.roofA < 0.02) continue;
      ctx.globalAlpha = sh.roofA;
    ...
    if (inSh && nightA > 0.05) {
      ...
      grd.addColorStop(1, `rgba(${p.glow},0)`);
      ctx.fillStyle = grd; ctx.fillRect(gx - 84, gy - 84, 168, 168);
    }
    // fireflies
    const ffA = Math.max(nightA, Math.min(1, finaleT / 3));
    if (ffA > 0.2) for (const ffy of fireflies) {
  ...
  function drawObj(o, tx, ty, ff) {
    ...
    if (o.type === 'tree' || o.type === 'stump' || o.type === 'boulder' || o.type === 'bush' || o.type === 'fire') shadow(X + 16, Y + T - 3, o.type === 'tree' ? 13 : 10);
    if (o.type === 'tree') ctx.drawImage(sp.props.tree, X - 8 + sh, Y - 30);
      ...
      ctx.fillStyle = pal.ui.bg; ctx.fillRect(o.ax * T + 56, o.ay * T - 6, 48, 7);
      ctx.strokeStyle = pal.ui.border; ctx.strokeRect(o.ax * T + 56.5, o.ay * T - 5.5, 47, 6);
      ctx.fillStyle = pal.ui.accent; ctx.fillRect(o.ax * T + 58, o.ay * T - 4, Math.round(44 * st), 3);
    }
  }
  function drawDrop(d) {
  ...
  function drawHUD() {
    ...
    let x = 5, cyy = 5;
    const t = tod();
    const dayIcon = t >= 360 && t < 1140 ? 'sun' : 'moon';
    const chips = [[dayIcon, clockLabel()], ['stick', inv.stick], ['stone', inv.stone], ['wood', inv.wood]];
    ...
    if (hints && stage < QUESTS.length) {
      const txt = stage === lastQ ? 'HOME' : QUESTS[stage].text;
      const maxW = VW - 24;
      ...
      ctx.fillStyle = stage === lastQ ? p.ui.accent : p.ui.text;
      ctx.textAlign = 'center';
  ...
  const handle = {
    start(k) {
      if (k && PALETTES[k]) { palKey = k; pal = PALETTES[palKey]; sp = getSprites(palKey, hairKey); }
      phase = 'game'; sfx.ensure(); sfx.quest(); saveSettings(); pushUi();
    },
    action() { if (phase === 'game') action(); },
    toggleCraft, craftItem, cancelPlace,
    cyclePalette,
    setPalette(k) { if (!PALETTES[k]) return; palKey = k; pal = PALETTES[palKey]; sp = getSprites(palKey, hairKey); saveSettings(); pushUi(); },
    setHair(k) { if (!HAIRS.some(h => h.key === k)) return; hairKey = k; sp = getSprites(palKey, hairKey); saveSettings(); pushUi(); },
    toggleSound,
    setHints(v) { hints = !!v; },
